...
`TemplateBlock` is a SMS++ :Block for ...


## Getting started

These instructions will let you build the `TemplateBlock` module on
your system.
...
```cmake
find_package(TemplateBlock)
target_link_libraries(<my_target> SMS++::TemplateBlock)
```
...
Each executable using `TemplateBlock` has to include a "main makefile" of
...
can [open a new issue](https://gitlab.com/smspp/templateblock/-/issues/new).
